...
export default function HomePage() {
  ...
  if (userLoading || !user) {
    return (
      <div className="fridge-loading">
        <div className="fridge-loading-dot" />
      </div>
    );
  }
  ...
  async function handleStartShopping() {
    await startSession(user!.id, displayName);
  }

  async function handleEndShopping() {
    if (session) await endSession(session.id);
  }

  async function handleCheck(itemId: string) {
    await checkItem(itemId, user!.id, displayName);
  }

  async function handleLock(itemId: string) {
    if (session) {
      await lockItem(itemId, user!.id, displayName, session.id);
    }
  }
    ...
    <div className="fridge-page">
      {session && (
        <ShoppingBanner
          ...
          onEndSession={handleEndShopping}
        />
      )}
      ...
      <header className="fridge-header">
        ...
        <div className="fridge-header-right">
          ...
          {!isInSession && (
            <button className="btn-shop" onClick={handleStartShopping}>
              🛒 Shop
            </button>
          )}
            ...
            {PRIORITY_ORDER.map((p) => {
              ...
              return (
                <div
                  ...
                  style={{
                    '--section-bg': cfg.bg,
                    '--section-border': cfg.border,
                    '--section-color': cfg.color,
